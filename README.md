...
- **云存储（夸克网盘）**：载荷（.pmp/.zip…）归档到夸克，**本地只留元数据 + 预览图 + 地址.txt**
  ...
  · **预览图（封面）也一起上云**：归档时先把「只在分类目录里的同级大图」复制进 mod 文件夹（自包含），
    再把图连同载荷一起传到云端并记账 —— **新电脑 / 重装 / 只从云端取回**时图能跟着回来
    （以前只传载荷：图既没上云、又不在 mod 文件夹里 → 推给插件时无图可写，游戏里那条 mod 目录永远是空的）
  · **缺图会自愈**：安装到游戏 / 补封面 / 诊断封面前，只要库里没图，就自动去云端目录把图取回库里
  ...
  · **与网盘对账**：以**云端实况**为准，重建每条的「已归档」状态、云端路径、大小和云端载荷清单 ——
    换电脑后 / 索引库丢了云状态 / 你在网盘里手工整理过，点一下就全认回来了（先「只看不改」预览）
...
- **封面出问题先点「诊断封面」**：管理器把整条链路摊开 —— 找到的图 / 转好的 WebP / 转好的 JPEG /
  游戏里那条 mod 的 `cover.webp` 与 `meta.json.Image`（插件自带 `/cover-check`，一眼看出卡在哪步）
...
| 组件 | 版本 |
|---|---|
| 管理器 ModManager | v2.23.2 |
| 游戏插件 ModBridge | v0.2.12（要求 ≥ v0.2.6；**封面塞进包**需要 ≥ v0.2.12） |
| 浏览器拓展 | v1.0.0 |
