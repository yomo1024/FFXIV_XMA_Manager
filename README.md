...
- 右侧详情**「操作」放最上面**：主动作两列等宽对齐（从站点更新／安装到游戏／上传新文件替换），
...
- **详情布局照 XMA 的 mod 页分五个区**，两列宽度比 **5:2**（侧栏最窄 220px）；
  ...
  上传新文件替换 → 图片数·载荷大小·站点版本）⑤ **元信息**（分类·更新时间·发布日期·类型·影响替换·种族·性别·标签·地址）
...
- **云存储（夸克网盘）**：载荷（.pmp/.zip…）归档到夸克，**本地只留元数据 + 预览图 + 地址.txt**
  ...
  · **缺图会自愈**：安装到游戏前，只要库里没图，就自动去云端目录把图取回库里
