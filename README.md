...
- **详情三档宽度**（操作卡右上角切换，或**双击列表任意一行**直接进「半屏」）：
  **窄栏**（默认 400px，边翻列表边看）／**半屏**（列表与详情各占一半，图更大、影响/替换一行就放得下）／
  **全屏**（详情铺满窗口，照 XMA mod 页排布：标题条横跨整行，下面左大图 + 右操作/元信息）；Esc 逐级退回
